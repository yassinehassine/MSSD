...
import { ReservationService, ReservationRequestDto } from '../../services/reservation.service';
...
export class Calendar implements OnInit {
  ...
  reservationForm: ReservationRequestDto = { calendarId: 0, visitorName: '', visitorEmail: '', visitorPhone: '', numberOfPeople: 1 };
  ...
  submitReservation() {
    ...
    this.reservationService.createReservation(this.reservationForm).subscribe({
      next: (reservation) => {
        this.reservationSuccess = 'Reservation successful!';
        this.reservationLoading = false;
        // Update event capacity in real time
        if (this.reservationEvent) {
          this.calendarService.getCalendarById(this.reservationEvent.id).subscribe({
            next: (updatedEvent) => {
              const idx = this.calendars.findIndex(e => e.id === updatedEvent.id);
              if (idx !== -1) {
                this.calendars[idx] = updatedEvent;
                this.applyFilters();
              }
      ...
      error: (err) => {
        this.reservationError = err.error?.message || 'Reservation failed.';
        this.reservationLoading = false;
      }
    });
  }
